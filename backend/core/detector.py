    ...
    def analyze_event(self, event):
        ...
        with self.lock:
            ...
            if event['event_type'] == 'moved':
                ...
                renames = sum(len(v) for k, v in self.rename_log.items() if now - k < RAPID_CHANGE_WINDOW)
            ...
            # Entropy check (only for modified/created files)
            ...
            if len([t for t, _ in self.event_log if now - t < RAPID_CHANGE_WINDOW]) > HIGH_IO_THRESHOLD:
                return {
                    'threat': True,
                    'reason': f"High I/O burst detected",
                    'severity': 'medium'
                }
            # No threat detected
            return {
